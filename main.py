...
# Initialize session state for rubric saved status
if "rubric_saved" not in st.session_state:
    st.session_state.rubric_saved = False
...
def main():
    ...
    with tabs[2]:
        ...
        if rubric_dict:
            ...
            st.session_state.rubric_saved = True  # Mark rubric as saved

        # Show the form for uploading ZIP and datasets only if the rubric is saved
        if st.session_state.rubric_saved:
            with st.form("upload_form"):
                uploaded_zip_file = st.file_uploader(
                    "Upload Student Submissions (ZIP)", type=["zip"]
                )
                uploaded_datasets = st.file_uploader(
                    "Upload Datasets (ZIP)", type=["zip"]
                )
